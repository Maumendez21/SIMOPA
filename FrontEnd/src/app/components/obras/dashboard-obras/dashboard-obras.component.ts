...
@Component({
  ...
  templateUrl: './dashboard-obras.component.html',
  styleUrls: ['./dashboard-obras.component.css']
})
export class DashboardObrasComponent {
  ...
  constructor(private app: AppComponent, private router: Router, private shared: SharedService, private pbrService: PbrService) {
    if (this.app.auth) {
      ...
      this.app.setTitle("Dashboard - Obras");
  ...
  prepareBreadcrumb() {
    this.shared.activePage.emit("Dashboard Obra Publica");
  ...
  loadData() {
    ...
    this.shared.updateStream$.subscribe(data => {
      this.pbrService.getDashboardExpedientes("obrapublica", this.ejercicio, this.auditor)
        .subscribe(response => {
  ...
  loadDataCharts() {
    this.shared.updateStream$.subscribe(data => {
      this.pbrService.getChartsDashboard("obrapublica", this.ejercicio, this.auditor)
        .subscribe(response => {
